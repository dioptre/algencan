...
C     LOCAL SCALARS
      ...
      call ma27ad(nsys,hnnz,hlin,hcol,iw,nnzmax,ikeep,iw1,nsteps,0,
     +icntl,cntl,info,ops)
...
C     LOCAL SCALARS
      ...
      call ma27bd(nsys,hnnz,hlin,hcol,fact,nnzmax,iw,nnzmax,ikeep,
     +nsteps,maxfrt,iw1,icntl,cntl,info)
      ...
      if ( info(1) .eq. 0 ) then
        ...
        ! INSUFFICIENT DOUBLE PRECISION WORKING SPACE
          ...
          if ( iprintctl(3) ) then
              write(* ,9020) nnzmax,info(2)
              write(10,9020) nnzmax,info(2)
          end if
        ...
        ! INSUFFICIENT INTEGER WORKING SPACE
          ...
          if ( iprintctl(3) ) then
              write(* ,9030) nnzmax,info(2)
              write(10,9030) nnzmax,info(2)
          end if
...
C     LOCAL SCALARS
      ...
      call ma27cd(nsys,fact,nnzmax,iw,nnzmax,w,maxfrt,sol,iw1,nsteps,
     +icntl,info)
...
C     LOCAL SCALAR
      ...
      if ( job .eq. 'T' .or. job .eq. 't' ) then
          ...
          call ma27qd(nsys,fact,nnzmax,iw(2),nnzmax-1,w,maxfrt,sol,
     +    iw1,nblk,latop,icntl)
          ...
          call ma27rd(nsys,fact,nnzmax,iw(2),nnzmax-1,w,maxfrt,sol,
     +    iw1,nblk,latop,icntl)
...
C     LOCAL ARRAYS
      integer iwtmp(nnzmax),iw1tmp(2*nsysmax),ikeeptmp(3*nsysmax)
      ...
      call ma27ad(nsys,hnnz,hlin,hcol,iwtmp,nnzmax,ikeeptmp,iw1tmp,
     +nstepstmp,0,icntl,cntl,info,opstmp)
      ...
      call ma27bd(nsys,hnnz,hlin,hcol,fact,nnzmax,iwtmp,nnzmax,
     +ikeeptmp,nstepstmp,maxfrttmp,iw1tmp,icntl,cntl,info)
      ...
      if ( info(1) .eq. 0 .or. info(1) .eq. 1 ) then
        ...
        ! INSUFFICIENT DOUBLE PRECISION WORKING SPACE
          ...
          if ( iprintctl(3) ) then
              write(* ,9010) nnzmax,info(2)
              write(10,9010) nnzmax,info(2)
          end if
      ...
      call ma27cd(nsys,fact,nnzmax,iwtmp,nnzmax,w,maxfrttmp,sol,
