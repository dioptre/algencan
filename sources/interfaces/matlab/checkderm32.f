...
C     LOCAL ARRAYS
      integer plhs(1),prhs(2)
      double precision x(nmax)
      character*159 warn

C     EXTERNAL FUNCTIONS
      external drand
      integer mxGetString,mxCreateCharMatrixFromStrings
      ...
      prhs(1) = mxCreateCharMatrixFromStrings(1, warn)

      call mexCallMATLAB(0,plhs,1,prhs,'fprintf')

      call mxDestroyArray(prhs(1))

      prhs(1) = mxCreateCharMatrixFromStrings(1,'')
      prhs(2) = mxCreateCharMatrixFromStrings(1,'s')
...
C     CHECK OBJECTIVE FUNCTION GRADIENT
      ...
      stat = mxGetString(plhs(1),answer,1)
