...
C     and R, the goal is to solve the feasibility problem:
C
C     d( pi, pj )^2 >= ( ri + rj )^2, for all i < j,
C
C     d( 0, pi )^2 <= ( R - ri )^2, for all i.
C
...
C     COMMON BLOCKS
      common /packdata/ pair,nite,ndim,iterad,objrad
...
C     SET PROBLEM DATA
     ...
     +           'items (nite): '
      ...
      write(*,*) 'Radii of circular items and object, respectively:'
      read(*,*) iterad,objrad
...
C     COMMON BLOCKS
      ...
      else if ( nite + 1 .le. ind .and.
     +          ind .le. nite + nite * ( nite - 1 ) / 2 ) then
