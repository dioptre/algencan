...
C     LOCAL SCALARS
      integer i,j
      double precision dist,objradub,seed,tmp
...
C     Initialize regions
      ...
      objradub = sqrt( 2.0d0 * iterad ** 2 * nite )

      nreg = ceil( objradub / iterad )
...
C     Initial point
      ...
      x(n) = sqrt( iterad ** 2 * nite * ( 1.0d0 + drand(seed) ) )

      tmp = ( x(n) - iterad )

      i = 1
 10   if ( i .le. nite ) then

          dist = 0.0d0
          do j = 1,ndim
              x(ndim*i+1-j) = - tmp + 2.0d0 * tmp * drand(seed)
              dist = dist + x(ndim*i+1-j) ** 2
          end do

          if ( dist .le. tmp ** 2 ) then
              i = i + 1
          end if
          go to 10
...
C     recommended.
 ...
 7000 format(/,' Welcome to GENPACK !!!',/
     ...
     +       /,' circles into a circular region, minimizing the area',
     +         ' of the region.',/)
...
C     EXTERNAL FUNCTIONS
      ...
      if ( 1 .le. ind .and. ind .le. nite ) then
          c = belong(n,x,ind)
      else if ( ind .eq. nite + 1 ) then
          c = overlap(n,x)
      end if
...
C     COMMON BLOCKS
      ...
      do i = 1,nite
          ...
          dist = 0.0d0
          do k = 1,ndim
              dist = dist + x(( i - 1 ) * ndim + k) ** 2
          end do
          dist = sqrt( dist )

          maxaloc = max( maxaloc, dist - ( x(n) - iterad ) )
...
C     SCALING
      scale = 10.0d0 / ( 2.0d0 * x(n) )
      write(10,10) scale

C     CIRCULAR OBJECT
      write(10,20) 2.0d0 * x(n),2.0d0 * x(n),0.0d0,0.0d0

C     CIRCULAR ITEMS
      do i = 1,nite
          write(10,20) 2.0d0 * iterad,2.0d0 * iterad,x(2 * i - 1),
     +                 x(2 * i)
      end do
...
C     NON-EXECUTABLE STATEMENTS
  ...
  10  format(/,'Circular object radius        = ',F16.10,
...
C     COMMON BLOCKS
      ...
      i = 1 + int( ( x + nreg * iterad ) / ( 2.0d0 * iterad ) )
      i = min( max( 1, i ), nreg )

      j = 1 + int( ( y + nreg * iterad ) / ( 2.0d0 * iterad ) )
      j = min( max( 1, j ), nreg )
...
C     COMMON BLOCKS
      ...
      belong = - ( x(n) - iterad ) ** 2
      do i = 1,ndim
          belong = belong + x(ndim * ( ind - 1 ) + i) ** 2
      end do
...
C     COMMON BLOCKS
      ...
      do i = 1,ndim
          gind(i) = ndim * ( ind - 1 ) + i
          gval(i) = 2.0d0 * x(ndim * ( ind - 1 ) + i)
      end do

      gind(nnzg) = n
      gval(nnzg) = - 2.0d0 * ( x(n) - iterad )
